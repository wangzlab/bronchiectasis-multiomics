...
dat.pval <- fread("NPD_Health_HostT_BSI.txt", data.table = F); head(dat.pval)
...
dat.need <- merge(dat.pval %>% select(-Commensal, -Health), 
                  dat.avg %>% select(Commensal, Health), by.x="Feature.full", by.y = 0)
head(dat.need)
dat.need <- dat.need %>% arrange(Commensal)
dat.need$dir <- ifelse(dat.need$Direction == "Down","-","+")

dat.hm <- dat.need %>% select(Feature.full, dir, Commensal, Health) %>% reshape2::melt(id.var=c("Feature.full","dir"), value.name = "avg")
head(dat.hm)
...
p1 <- ggplot(dat.hm) +
  ...
  scale_fill_gradient2(low = "#E6949A", 
                       ...
                       mid = "white",
                       midpoint =  0, 
                       limits = c(-0.58, 0.65), # unify scales for NPD and PPMD
                       na.value="gray") +
  theme_bw() +
...
p2 <- ggplot(dat.need) +
  ...
  scale_fill_distiller(palette="Greys",
                       limits=c(2.4, 14.6), # unify scales for NPD and PPMD
                       direction = 1 )
p2
...
p3 <- ggplot(dat.need) +
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank())+
  scale_fill_distiller(limits=c(0,0.22),  # unify scales for NPD and PPMD
                       palette="Blues",
                       direction = 1 )
...
p4 <- ggplot(dat.need) +
  ...
  scale_fill_gradient2(low = "#E6949A", 
                       ...
                       mid = "white",
                       midpoint =  0, 
                       limits = c(-8.3,5.0), # unify scales for NPD and PPMD
                       na.value="gray")
...
ggsave(filename = "3b_NPD.health_hostTpathway.hm.pdf", width = 8, height = 4.5)
