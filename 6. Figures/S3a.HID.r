...
dat.pval <- fread("HID_Healthy_Module_BSI.txt", data.table = F)
...
dat.need <- merge(dat.pval, dat.avg %>% dplyr::select(Hi, Health), by.x="Feature", by.y = 0)
dat.need <- dat.need %>% arrange(Hi)
dat.need$dir <- ifelse(dat.need$Direction == "Down","-","+")

dat.hm <- dat.need %>% dplyr::select(Feature, dir, Hi, Health) %>% reshape2::melt(id.var=c("Feature","dir"), value.name = "avg")
head(dat.hm)
...
p1 <- ggplot(dat.hm_HID) +
  ...
  scale_fill_gradient2(low = "#E6949A", 
                       ...
                       mid = "white",
                       midpoint =  0, 
                       # limits = c(-0.7,0.71),# NPD和PPMD统一scale
                       na.value="gray") +
  theme_bw() +
...
dat.need_HID <- dat.need
p2 <- ggplot(dat.need_HID) +
  ...
  scale_fill_distiller(limits=c(1.6,7), # NPD和PPMD统一scale
                       palette="Greys",
                       direction = 1 )
...
p3 <- ggplot(dat.need_HID) +
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank())+
  scale_fill_distiller(limits=c(0,1),  # NPD和PPMD统一scale
                       palette="Blues",
                       direction = 1 )

# plot of estimate
p4 <- ggplot(dat.need_HID) +
  ...
  scale_fill_gradientn(
    ...
    values = scales::rescale(
      c(-0.37, 0, 0.29),  # 这三个数据值
      from = c(-0.37, 0.29)  # 对应到范围
    ),
    limits = c(-0.37, 0.29),
    na.value = "gray",
    oob = scales::squish
...
ggsave(filename = "S3a.HID.health_module.hm.pdf", width = 8, height = 4.5)
