...
dat.pval <- fread("HID_Health_MetaB_BSI(1).txt", data.table = F)
head(dat.pval)

#feature match list
metabMatch <- fread("../_dataGZ/metab_matchlist.txt", data.table = F, header = F)
dat.pval$V1 %in% metabMatch$V2
dat.pval$Feature <- sapply(dat.pval$V1, function(x) metabMatch$V1[which(metabMatch$V2 == x)])
dat.pval$Feature <- paste0("ME", dat.pval$Feature)
...
dat.pval$Feature %in% colnames(dat)
...
length(c(sps.Hi, sps.Health))
sps.keep <- c(sps.Hi, sps.Health)[c(sps.Hi, sps.Health) %in% rownames(tmp)] # metaB 本来只有99个？
tmp  <- tmp[sps.keep,]
...
dat.need <- merge(dat.pval %>% select(-Health,-HID),
                  dat.avg %>% select(Hi, Health), by.x="Feature", by.y = 0)
head(dat.need)
dat.need <- dat.need %>% arrange(Hi)
dat.need$dir <- ifelse(dat.need$Direction == "Down","-","+")


dat.hm <- dat.need %>% select(Feature,dir, Hi, Health) %>% reshape2::melt(id.var=c("Feature","dir"), value.name = "avg")
head(dat.hm)
dat.hm$dir[dat.hm$variable == "Health"] <- NA
dat.hm$Feature <- factor(dat.hm$Feature, levels = dat.need$Feature)
dat.hm$variable <- factor(dat.hm$variable, levels = c("Health","Hi"))
...
p2 <- ggplot(dat.need) +
  ...
  scale_fill_distiller(palette="Greys",
                       # limits=c(3.79,6.5),  # NPD和PPMD统一scale
                       direction = 1 )
...
p3 <- ggplot(dat.need) +
  ...
  scale_fill_gradient2(low = "#E6949A", 
                       ...
                       mid = "white",
                       midpoint =  0, 
                       # limits = c(-14.1,13.6), # NPD和PPMD统一scale
                       na.value="gray")
...
pD.md <- md.compositions %>%  filter(Module %in%  dat.need$Feature) 
pD.md$Module <- factor(pD.md$Module, levels = dat.need$Feature)
...
p5 <- ggplot(dat.need) +
  ...
  scale_fill_distiller(
    limits=c(0,0.29),  # NPD和PPMD统一scale
    palette="Blues",
    direction = 1 )

library(ggpubr)
ggarrange(p1,p2,p3,p5,p4,  ncol = 5, widths = c(0.25,0.1, 0.11,0.1,0.44), nrow = 1)
ggsave(filename = "S4b.HID.health_FC.hm_moduleComposition.pdf", width = 12, height = 4.5)
