...
dat <- fread("../_dataGZ/metag_ko.txt",data.table = F)
head(dat[,1:6])
...
P0 <-  ggplot(site,aes(X1,X2))+
  ...
  # scale_x_continuous(limits = c(-0.52,0.4)) +   # limits看了pcoa和pc1.density图后再回来改
  # scale_y_continuous(limits = c(-0.42,0.3)) +   # limits看了pcoa和pc2.density图后再回来改
...
P0
ggsave(filename = "2a.metagFunction.pdf", width = 5.5, height = 3.5)
