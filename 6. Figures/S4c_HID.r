...
dat.pval <- fread("HID_Health_HostT_BSI.txt", data.table = F);head(dat.pval)
...
dat.need <- merge(dat.pval %>% select(-Pa, -Health), dat.avg %>% select(Hi, Health), by.x="Feature", by.y = 0)
head(dat.need)
...
p3 <- ggplot(dat.need) +
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank())+
  scale_fill_distiller(limits=c(0,0.22),  
                       palette="Blues",
                       direction = 1 )
p3
...
ggsave(filename = "S4c.Hi.health_hostTpathway.pdf", width = 8, height = 4.5)
