...
dat.pval <- fread("PAD_Health_MetaB_BSI(1).txt", data.table = F);head(dat.pval)

ModuleMatch <- fread("../_dataGZ/metab_matchlist.txt", data.table = F,header = F)
dat.pval$Feature <- sapply(dat.pval$V1, function(x) ModuleMatch$V1[which(ModuleMatch$V2 == x)])
...
length(c(sps.Pa, sps.Health))
sps.keep <- c(sps.Pa, sps.Health)[c(sps.Pa, sps.Health) %in% rownames(tmp)] 
tmp  <- tmp[sps.keep,]
...
dat.need <- merge(dat.pval%>% select(-PAD, -Health), dat.avg %>% select(Pa, Health), by.x="Feature", by.y = 0)
head(dat.need)
...
ggsave(filename = "S4b.PAD.health_FC.hm_moduleComposition.pdf", width = 12, height = 4.5)
