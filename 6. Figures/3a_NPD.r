...
dat.pval <- fread("NPD_Health_MetaB_BSI.txt", data.table = F);head(dat.pval)
...
sps.keep <- c(sps.Commensal, sps.Health)[c(sps.Commensal, sps.Health) %in% rownames(tmp)] # metaB 本来只有99个？
tmp  <- tmp[sps.keep,]
...
#dat.need$mlog10P <- -log10(dat.need$glm.p)
...
ggsave(filename = "3a_NPD.health_FC.hm_moduleComposition.pdf", width = 12, height = 4.5)
