...
open (IN, "metab_module_assign.txt");## assign metabolite ID to metab module based on WGCNA
while (<IN>) {
	chop;
	@a=split("\t",$_);
...
open (IN, $ARGV[0]); ## input HALLA all_associations.txt table
while (<IN>) {
	chop;
	@a=split("\t",$_);
