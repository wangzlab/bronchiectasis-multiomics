...
dat <- fread("../_dataGZ/metaT_module.txt", data.table = F)

colnames(dat) %in% meta_origin$`#NAME` 
IDmatch <- fread("../_dataGZ/metat_match.txt", data.table = F,header = F)
colnames(dat) %in% IDmatch$V1
IDmatch$V2 %in% meta_origin$`#NAME`

colnames(dat)[-1] <- sapply(colnames(dat)[-1], function(x) IDmatch$V2[which(IDmatch$V1 == x)])
colnames(dat) %in% meta_origin$`#NAME` 
...
for(cp in comparison_list){  
  ...
  pval_res$DataSource = "metaT_module"
  ...
  write.table(pval_res,
              file = paste0("metaT_module_", paste(cp[1],cp[2], sep = ".vs."), ".txt"),
