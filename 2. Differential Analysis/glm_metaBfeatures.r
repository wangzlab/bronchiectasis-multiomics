...
grouping_cols <- colnames(grouping)[grepl("group", colnames(grouping), ignore.case = TRUE)]
...
dat <- fread("../_dataGZ/metaB_feature.txt", data.table = F)
...
write.table(feature_abb_df, file = "metaB_feature.abb.txt", sep = "\t",quote = F, row.names = F)
...
for(cp in comparison_list){  
  ...
  pval_res$DataSource = "metaB_feature"
  ...
  write.table(pval_res,
              file = paste0("metaB_feature_", paste(cp[1],cp[2], sep = ".vs."), ".txt"),
...
Stable.Exacerb_pval$DataSource = "metaB_feature"
...
write.table(Stable.Exacerb_pval,
            file = "metaB_feature_Stable.vs.Exacerbations.txt",
            sep="\t", quote = F, row.names = F)
